...
export async function getOrCreateIdentity({ email, anonId }) {
  ...
  const { data, error } = await supabase.from('users_quota').select('*').eq('id', id).maybeSingle();
  if (data) return data;
  const plan = email ? 'free' : 'free';
...
export async function syncPlanFromSubscriptions(email) {
  ...
  const credits = PLAN_CREDITS[plan];
  // rotate month bucket if needed
  const mk = monthKey();
  const { data: uq } = await supabase.from('users_quota').select('*').eq('id', email.toLowerCase()).maybeSingle();
  if (!uq) {
    await supabase.from('users_quota').insert({ id: email.toLowerCase(), email: email.toLowerCase(), plan, month_key: mk, credits_remaining: credits, last_updated: new Date().toISOString() });
    return { plan, credits_remaining: credits, month_key: mk };
  }
  const newRow = { plan };
  if (uq.month_key !== mk) {
    newRow.month_key = mk;
    newRow.credits_remaining = credits;
  }
...
export async function requireCredits({ email, anonId, cost = 1, action = 'generic' }) {
  ...
  // rotate month if needed & sync plan if email
  if (email) await syncPlanFromSubscriptions(email);

  const { data: row, error } = await supabase.from('users_quota').select('*').eq('id', id).maybeSingle();
  if (error || !row) return { ok: false, reason: 'quota-read-failed' };
...
export async function getCredits(emailOrAnon) {
  const supabase = getServerSupabase();
  if (!supabase) return { remaining: 0, plan: 'free' };
  const id = (emailOrAnon || '').toLowerCase();
