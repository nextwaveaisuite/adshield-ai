...
export async function setPlanByEmail(email, { plan, status, current_period_end, stripe_customer_id, stripe_subscription_id }) {
  ...
  return await supabase.from('subscriptions').upsert({
    email: email.toLowerCase(), plan, status,
    current_period_end: current_period_end ? new Date(current_period_end) : null,
    stripe_customer_id: stripe_customer_id || null, stripe_subscription_id: stripe_subscription_id || null
  }, { onConflict: 'email' }).select().single();
}
export async function getPlanByEmail(email){
  const supabase=getServerSupabase(); if(!supabase) return {plan:'free', status:'inactive'};
