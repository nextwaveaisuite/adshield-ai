...
export default function Pricing({ email }) {
  ...
  async function checkout(priceVar) {
    setLoading(true);
    const res = await fetch('/api/stripe/create-checkout-session', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email, price: process.env.NEXT_PUBLIC_DUMMY || undefined })
        .replace('NEXT_PUBLIC_DUMMY', '') // no-op, ensure JSON stays valid
    });
