  ...
  return (
    <main style={{fontFamily:'system-ui, Arial, sans-serif', padding:24, maxWidth:1080, margin:'0 auto'}}>
      ...
      <section style={{marginBottom:16, color:'#444'}}>
        Signed in as <b>{user?.email}</b> — Plan: <b>{sub?.plan || 'free'}</b> ({sub?.status || 'inactive'}) • Credits: <b>{credits.remaining < 0 ? '∞' : credits.remaining}</b>
        {low && <span style={{marginLeft:12, color:'#b45309'}}>Low credits — consider <Link href="/pricing">upgrading</Link>.</span>}
      </section>
      <Dashboard />
    </main>
  );
