...
export default async function handler(req,res){
  ...
  const stripeKey=process.env.STRIPE_SECRET_KEY; if(!stripeKey) return res.status(500).json({ error:'Missing STRIPE_SECRET_KEY' });
  ...
  try{
    const session=await stripe.checkout.sessions.create({
      ...
      customer_email: email || undefined, allow_promotion_codes: true, metadata:{ app:'adshield-ai' }
    });
