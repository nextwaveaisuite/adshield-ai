...
export default function Dashboard(){
  ...
  const chartData=useMemo(()=>({ labels, datasets:[ {label:'Posts', data: rows.map(r=>r.posts||0)}, {label:'Clicks', data: rows.map(r=>r.clicks||0)}, {label:'Leads', data: rows.map(r=>r.leads||0)}, {label:'Sales', data: rows.map(r=>r.sales||0)} ] }),[rows,labels]);
  const sorted=useMemo(()=>[...rows].sort((a,b)=>(b[sortKey]||0)-(a[sortKey]||0)),[rows,sortKey]);
  if(loading) return <p>Loading metrics…</p>; if(error) return <p style={{color:'crimson'}}>{error}</p>; if(!rows.length) return <p>No data yet. POST /api/collect to create some events.</p>;
  return (<div style={{display:'grid',gap:24}}>
    ...
    <div style={{background:'#fff',padding:16,borderRadius:12,boxShadow:'0 1px 4px rgba(0,0,0,.06)'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}><h3>Aggregated Metrics</h3><label>Sort by:&nbsp;<select value={sortKey} onChange={e=>setSortKey(e.target.value)}><option value="clicks">Clicks</option><option value="posts">Posts</option><option value="leads">Leads</option><option value="sales">Sales</option></select></label></div>
